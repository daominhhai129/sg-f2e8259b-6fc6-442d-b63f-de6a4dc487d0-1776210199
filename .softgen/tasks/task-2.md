---
title: "Implement users and business cards management UI"
status: "todo"
priority: "high"
type: "feature"
tags: ["users", "business-cards"]
created_by: "agent"
created_at: "2026-04-14T16:05:59Z"
position: 2
---

## Notes

Extend the dashboard with a focused management UI for individual users and their digital business cards. This includes a detail panel, editable fields for card content, and basic search/filter capabilities, all running purely on the frontend with mock state.

## Checklist

- [ ] Create a detail panel for the selected user showing a digital business card preview
- [ ] Add editable fields for card content (name, title, company, bio, links)
- [ ] Wire row selection in the users table to update the detail panel
- [ ] Add basic search and/or filter for users by name, email, or status